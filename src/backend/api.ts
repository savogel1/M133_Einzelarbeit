...
const products: Product[] = JSON.parse(await Deno.readTextFile("./src/backend/classes/products.json"));
...
router
    ...
    .post(`/cart/addAmount/:id`, async (ctx) => {    
    ...
    .post(`/cart/removeAmount/:id`, async (ctx) => {    
        let cart = await getCart(ctx);
        let product = getProduct(ctx.params.id!);
